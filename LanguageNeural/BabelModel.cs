...
//                        s_LargeModel.Add("is", _AllModel["is"]);
        ...
        public static BabelModel _AllModel
		{
			get
			{
                lock (s_LoaderLock)
                {
                    if (s_AllModel == null)
                    {
                        //s_AllModel = LoadFromFile(@"c:\ass.bin");
                        Assembly a = Assembly.GetExecutingAssembly();
                        //using (Stream s = a.GetManifestResourceStream("DialogueMaster.Babel.models.all.model"))
                        using (Stream s = a.GetManifestResourceStream("LanguageNeural.models.all.model"))
                        {
    ...
    sealed class typeconvertor : SerializationBinder
    {
        public override Type BindToType(string assemblyName, string typeName)
        {
            ...
            if (assemblyName == "DialogueMaster.Babel, Version=1.6.0.5, Culture=neutral, PublicKeyToken=null")
            {
            ...
            if (typeName == "DialogueMaster.Babel.BabelModel")
            {
                typeName = typeName.Replace("DialogueMaster.Babel.BabelModel",
